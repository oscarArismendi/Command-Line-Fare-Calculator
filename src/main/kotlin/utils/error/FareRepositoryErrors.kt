...
sealed class FareRepositoryErrors(message: String): Exception(message) {
    ...
    class InvalidJourneyError: FareRepositoryErrors("Origin and destination cannot be the same")
    class InvalidTimeError(message: String): FareRepositoryErrors(message)
}
